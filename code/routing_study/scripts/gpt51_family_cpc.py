...
qwen 侧会自行改写成单花括号，gpt-5.1 侧则原样照抄，导致主解析器
...
OUTDIR = Path(os.environ.get("DSFLASH_OUTDIR", RESULTS / "topn_seeds_gpt51"))
MDT_OUTDIR = Path(os.environ.get("DSFLASH_MDT_OUTDIR",
                                 RESULTS / "topn_mdt_gpt51"))
QWEN_SEEDS = RESULTS / "topn_seeds"
QWEN_MDT = RESULTS / "topn_mdt"
SEEDS = [int(s) for s in os.environ.get("DSFLASH_SEEDS", "1,2,3,4,5").split(",")
         if s.strip()]
...
MAX_TOKENS = 4096
MAX_ATTEMPTS = 3
OUT_JSON = RESULTS / "gpt51_family_cpc.json"
OUT_MD = RESULTS / "gpt51_family_cpc.md"
PAIRS = [("MDT", "Ax1"), ("MDT", "P"), ("P", "Ax1"),
         ("MDT_qwen", "MDT"), ("Ax1_qwen", "Ax1"), ("P_qwen", "P")]
ARM_LABEL = {"Ax1": "A×1 (gpt-5.1)", "P": "P (gpt-5.1)",
             "MDT": "MDT (gpt-5.1)",
...
def render_md(meta, stats, splits_order):
    L = []
    L.append("# 闭源旗舰（gpt-5.1）CPC 扩展：A×1 / P / MDT\n")
    L.append(f"回应评审 (b)：主结果只来自 qwen3.8-flash 一个模型族。本表用 "
             f"gpt-5.1 重跑 CPC 的 A×1 / P（87 例 × {meta['n_seeds']} seeds）与 MDT "
             "（五角色 + 主持人，同族 gpt-5.1），并用主判官（GLM-5.3-flash × "
    ...
    L.append("`topn_cpc_promptv2.P_TOPN_SUFFIX` 是一段从未经过 `.format()` 的普通"
             ...
             "gpt-5.1 侧会原样照抄 → 主解析器 `extract_json` 失败 → top5 为空。\n")
    L.append(f"本实验**不修改提示词**（gpt-5.1 臂与主实验逐字节相同，两个模型族"
    ...
    if full:
        ...
        L.append(f"\n## 结论（gpt-5.1 族扩展，全量 87 例，{meta['n_seeds']} seeds）\n")
        for pair in ("MDT_vs_Ax1", "MDT_vs_P", "P_vs_Ax1"):
            L.append(f"- 族内：{line(pair, 1)}；{line(pair, 5)}")
        L.append("- 跨族一致性（qwen 族 vs gpt-5.1 族，同架构同提示词）：")
